...
class Kinetics400(tdata.Dataset):
    def __init__(self, path, split, transform):
        ...
        with open(csv_path) as fp:
            reader = csv.DictReader(fp)
            for item in reader:
                #print(f"Expected split: {csv_split}, Found in CSV: {item['split']}")
                assert csv_split == item["split"]
                name = "%s_%06d_%06d" % (
                    item["youtube_id"],
                    int(item["time_start"]),
...
class HMDB51(tdata.Dataset):
    ...
    def read_images(self, path, use_transform):
        ...
        for i in self.frames:
            image = Image.open(
                # revise becasuse of different dataset settings
...
class miniUCF101(tdata.Dataset):
    ...
    def read_images(self, path, use_transform):
        ...
        for i in self.frames:
            image = Image.open(
                # change for the different settings
                os.path.join(path, "img_{:05d}.jpg".format(i))
            )
    ...
    def __init__(self, path, split, transform):
        ...
        with open(json_path) as fp:
            ...
            for item in content:
                ...
                sample_dir = osp.join(path, 'rawframes', name)
                if (
                    not osp.exists(sample_dir)
                    or len(os.listdir(sample_dir)) != NUM_FRAMES
                ):
