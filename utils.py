...
def get_dataset(dataset, data_path, num_workers=0,img_size=(112,112),split_num=1,split_id=0,split_mode='mean'):
    ...
    if dataset in ['Kinetics400', 'Kinetics400_long']:
        ...
        dst_train = SSv2(path, split="train", transform=transform) # no augmentation
        dst_test  = SSv2(path, split="val", transform=transform)
        class_names = None
    ...
    if mode == 'train':
        ...
        for j_ in range(3):
            ...
            for i_batch, datum in enumerate(dataloader):
                ...
                acc = np.sum(np.equal(preds, lab.cpu().data.numpy()))
...
def preload_test_data(dst_test):
    """Preloads the test dataset into memory as tensors."""
    print("Preloading test dataset")
    all_images = []
    all_labels = []
    
    for i in tqdm.trange(len(dst_test)):
        _ = dst_test[i]
        all_images.append(_[0])
