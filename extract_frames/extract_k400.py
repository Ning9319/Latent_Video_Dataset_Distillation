...
def extract_frames_progress(video_path):
    in_pth  = os.path.join(input_dir, video_path).replace("\\", "/")
    out_dir = os.path.join(output_dir, video_path[:-4]).replace("\\", "/")
    os.makedirs(out_dir, exist_ok=True)
    out_pth = os.path.join(out_dir, "%02d.jpg").replace("\\", "/")
    
    extract_frames(in_pth, out_pth, fps=2, num_frames=8)


if __name__ == "__main__":  # <-- FIX: Add this guard
    for split_name in split_name_list:
        ...
        for video_name in os.listdir(root_path):
            if os.path.exists(os.path.join(output_dir, split_name, video_name[:-4]).replace("\\", "/")):
                continue
            #else:
            ...
            if not video_name.endswith(".mp4") or video_name[0]=='.':
                ...
                continue
            video_path = os.path.join(split_name, video_name).replace("\\", "/")
